...
class Unit(pygame.sprite.Sprite):
    ...
    def collision(self, cX, cY):
        ...
        # limites de la map
        if (cX < 0 or cX >= (GAME_DIMENSIONS[0] - self.size + 1) * BASE) or (
                cY < 0 or cY >= (GAME_DIMENSIONS[1] - self.size + 1) * BASE):
            return True
        ...
        for i in range(-rng, rng + self.size):
            ...
            for j in range(-rng, rng + self.size):
                ...
                for ob in self.board.board:
                    if legal(ob.x) <= x < legal(ob.x) + ob.size * BASE and legal(ob.y) <= y < legal(
                            ob.y) + ob.size * BASE and ob != self:
                        retour.append(ob)
        return retour
